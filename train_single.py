...
def train(
    ...
    num_epochs: int,
    num_layers: int, learning_rate: float, dropout: float, data_set_number: int
) -> None:
    """
    ...
    train_dataloader = ProvidenceDataLoader(
        train_data, batch_size=100, pin_memory=torch.cuda.is_available(), sampler=train_sampler
    )
    validation_dataloader = ProvidenceDataLoader(
        train_data, batch_size=100, pin_memory=torch.cuda.is_available(), sampler=valid_sampler
    )
