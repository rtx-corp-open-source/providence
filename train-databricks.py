...
def main(
    network_type=typer.Option("GRU", case_sensitive=False, help="Type of neural network to run 💻"),
    optimizer_type=typer.Option("Adam", case_sensitive=False, help="Type of optimizer to use. 👩‍🏫"),
    ...
    dataset_choice: TrainingDataset = typer.Option(
        TrainingDataset.nasa,
        case_sensitive=False,
        help="Set this to use one of the provided datasets",
    ),
):
    outdir = Path(
        "outputs",
        dataset_choice.value,
        parameterized_run_name(
            network_type,
            optimizer_type,
            num_layers,
            layer_width,
    ...
    new_train(
        ...
        callbacks=[
            ...
            IntervalMetricsVisualizer(
                every=ceil(num_epochs // 5),
                output_dir=outdir / "metrics_plots",
                logger=logger,
            ),
            WriteModelOutputs(
...
def search_hyperparameter_space(
    ...
    def train_model(training_config: Dict) -> Metrics:
        ...
        callbacks = [
            LearningCurveTracker(
                every=2, output_dir=outdir
            ),  # NOTE: inner call to plt.close('all') saves us in the long haul.
            EpochLoggerCallback(logger)
            if num_epochs < 100
            else NthEpochLoggerCallback(metrics_viz_frequency, logger),  # TODO(speed-up): log less frequently.
