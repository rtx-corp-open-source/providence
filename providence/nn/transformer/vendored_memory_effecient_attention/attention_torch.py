"""
Importing the open-source memory-effecient attention implementation. See the adjacent README.md for more.
...
def _query_chunk_attention(
    ...
    chunk_values, chunk_weights, chunk_max = map_pt(chunk_scanner, xs=torch.arange(0, num_kv, key_chunk_size))
...
def efficient_dot_product_attention(
    ...
    Args:
        query (Tensor): queries for calculating attention with shape of
            ``[batch..., q_length, num_heads, qk_depth_per_head]``.
        key (Tensor): keys for calculating attention with shape of
            ``[batch..., kv_length, num_heads, qk_depth_per_head]``.
        value (Tensor): values to be used in attention with shape of
            ``[batch..., kv_length, num_heads, v_depth_per_head]``.
        bias (Tensor): bias for the attention weights. This should be broadcastable to the
            shape ``[batch..., num_heads, q_length, kv_length]``.
            This can be used for incorporating padding masks, proximity bias, etc.
        mask (Tensor): mask for the attention weights. This should be broadcastable to the
            shape ``[batch..., num_heads, q_length, kv_length]``.
        ...
        bias_calc_fn: a bias calculation callback for each chunk, of form
            ...
            This can be used for incorporating causal masks, padding masks, proximity bias, etc.
        ...
        weights_calc_fn: a general attn_weights callback for each chunk, of form
            ...
            attn_weights has shape of ``[batch..., q_chunk_size, num_heads, k_chunk_size]``.
            This can be used to implement complex weights processing in a memory efficient way.
