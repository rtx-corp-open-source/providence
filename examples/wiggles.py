...
def load_model_from_saved_state_dict(path: str) -> nn.Module:
    model = prov_model(24, 120, n_layers=4, dropout=0.2)
    model.load_state_dict(torch.load(path, map_location=torch.device("cpu")))
    return model


def animate_weibull_curve_over_time(model: nn.Module, data: Tuple(torch.Tensor, torch.Tensor)) -> animation.FuncAnimation:
    """
