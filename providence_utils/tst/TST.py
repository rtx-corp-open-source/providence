...
from providence_utils.fastai_layers import Flatten, Pad1d, SigmoidRange, Transpose
from providence_utils.fastai_torch_core import Module
from providence_utils.fastai_utils import ifnone, pv
...
class _TSTEncoderLayer(Module):
    def __init__(
        ...
        self.ff = nn.Sequential(
            nn.Linear(d_model, d_ff), get_activation_fn(activation), nn.Dropout(dropout), nn.Linear(d_ff, d_model)
        )
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        # NOTE(stephen): this is highly reminiscent of the old implementation of core PyTorch's Transformer Encoder Layer
...
class _TSTEncoder(Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                _TSTEncoderLayer(
                    q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, dropout=dropout, activation=activation
                )
...
class TST(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Positional encoding
        # W_pos = torch.empty((q_len, d_model), device=default_device()) # NOTE: original
        W_pos = torch.empty(
            (q_len, d_model), device=use_gpu_if_available()
        )  # NOTE: modified, as use_gpu_if_available() has similar semantic
        ...
        # Encoder
        # NOTE(stephen): `q_len` doesn't do anything done the stack...
        self.encoder = _TSTEncoder(
            q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, dropout=dropout, activation=act, n_layers=n_layers
        )
        self.flatten = Flatten()  # NOTE: unusued

        # Head
        self.head_nf = q_len * d_model
        self.head = self.create_head(self.head_nf, c_out, act=act, fc_dropout=fc_dropout, y_range=y_range)

    def create_head(self, nf, c_out, act="gelu", fc_dropout=0.0, y_range=None, **kwargs):
    ...
    def forward(self, x: Tensor, mask: Optional[Tensor] = None) -> Tensor:  # x: [bs x nvars x q_len]
        # NOTE(stephen): mask doesn't do anything
