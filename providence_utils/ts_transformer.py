"""
Pulled from the paper's repository:
...
def model_factory(config, data):
    ...
    if (task == "imputation") or (task == "transduction"):
        if config["model"] == "LINEAR":
            ...
            return TSTransformerEncoder(  # noqa: F821
                feat_dim,
                max_seq_len,
                config["d_model"],
                config["num_heads"],
...
class FixedPositionalEncoding(nn.Module):
    ...
    The positional encodings have the same dimension asthe embeddings, so that the two can be summed. Here, we use sine and cosine
...
class TSTransformerEncoder(nn.Module):
    ...
    def forward(self, X, padding_masks):
        """Perform the forward pass over input data ``X``, masking appropriately with ``padding_masks``.
        Args:
