...
from providence_utils.callbacks import Callback, DeferrableEarlyStopping, EarlyStopping, EmergencyBrake, check_before_epoch
...
class TestDeferrableEarlyStopping:
    ...
    def test_core_usage(self):
        ...
        for i in range(test_defer):
            ...
            cb.after_epoch(epoch=i, losses=EpochLosses(i * 10, None), model=..., optimizer=..., dls=...)
        ...
        for i in range(test_patience):
            ...
            cb.after_epoch(epoch=..., losses=EpochLosses(i * 10, None), model=..., optimizer=..., dls=...)
...
class TestInteractions:
    ...
    def test_ebrake_deferrable_conflict__ebrake_wins(self):
        ...
        for _ in range(2):
            for cb in cbs:
                cb.after_epoch(losses=EpochLosses(training=50, validation=50), epoch=..., model=..., optimizer=..., dls=... )
    ...
    def test_ebrake_deferrable_conflict__ebrake_narrow_win(self):
        ...
        ebrake = EmergencyBrake(check_at=3, requisite_loss=50.)
        ...
        assert not should_terminate, f"Haven't check the callbacks again, even though the state change in {type(ebrake).__name__} warrants it"

        should_terminate, error_message = check_before_epoch(cbs)
        assert should_terminate, f"should terminate because the Ebrake wasn't satisfied, per its check at epoch {ebrake.check_at}"
