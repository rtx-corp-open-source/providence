...
EXPERIMENT_NAME = "/Users/40000889@azg.utccgl.com/Providence-Investigative-Research/NASA Transformer (Hybrid, Redux)"

EXPERIMENT_ID = create_or_set_experiment(EXPERIMENT_NAME)

ROOT_DIR = Path("/dbfs/FileStore/providence-benchmarking/nasa-transformer-hybrid-redux")
...
def performance_comparison_run():
    # this run started after the set_seed changed to return to setting the rng state
    experiment_id = create_or_set_experiment(EXPERIMENT_NAME.replace("Transformer", "Transformer Performance"))
    n_seeds_to_check = 1
    for _ in range(n_seeds_to_check):
        fixed_seed = pt.seed()
        for tfmr_type in [ProvidenceDeepMindTransformer, ProvidenceBertTransformer]:
